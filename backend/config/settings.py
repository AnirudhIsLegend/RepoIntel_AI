...
# For local development, fall back to SQLite if DB_HOST is not set.
...
# Production:
# FRONTEND_URL=https://repo-intel-ai.vercel.app
#
# rstrip('/') prevents:
# https://repo-intel-ai.vercel.app//auth/callback
...
THROTTLE_RATES = {
    ...
    'architecture': config(
        'THROTTLE_ARCHITECTURE',
        default='50/day',
    ),

    'repository_overview': config(
        'THROTTLE_OVERVIEW',
        default='300/day',
    ),
}
...
GEMINI_EMBEDDING_DIMENSIONS = config(
...
QDRANT_API_KEY = config(
    'QDRANT_API_KEY',
    default='',
)

# Embedding dimension must match Gemini embedding output.
